...
class NationalHte(RemAiv3):
    ...
    def load_db(self, tree4load=None):  # override parent class
        ...
        for usn, v in tree4load.items():
            ...
            self.tree[usn] = {
                "oname_fr": v["oname_fr"],
                "nht_period": v.get("misc", {}).get("nht_period", {}),
            }
...
def xtags(remaicfg):
    ...
    rlog.info("-" * 72)
    rlog.info(f"{remaicfg.year} :: tag '{organization.coll}' and '{student.coll}'.")
    rlog.info("-" * 72 + "\n")
...
def washd(remaicfg):
    ...
    rlog.info(f"{stuimp.coll} :: {len(stuimp.flist)} file(s)(就业方案导出) found.\n")
    ...
    rlog.info(
        f"{org4br.coll} :: set field 'parent_node' based on file, skip: {result['skip']}, set: {result['set']}, pc: {show_t(result['pc'],cr='no')}."
    )
    # print("=" * 144)
    ...
    rlog.info(
        f"{org4br.coll} :: set field 'parent_node' based on file-aqc, skip: {result['skip']}, set: {result['set']}, pc: {show_t(result['pc'],cr='no')}."
    )
    ...
    rlog.info(
        f"{org4br.coll} :: set field 'parent_node' based on '{org4usn.coll}', skip: {result['skip']}, set: {result['set']}, pc: {show_t(result['pc'],cr='no')}."
    )
    # print("=" * 144)
    # org4bizreg.show_tree(60)
    ...
    if result["ops4db"] == True:
        rlog.info(
            f"{org.coll} :: update done, [{result['result'].matched_count}] matched, [{result['result'].modified_count}] modified."
        )
    else:
        rlog.info(f"{org.coll} :: update done, no doc changed in db.")
    ...
    for listx in list2wd:
        ...
        if result["ops4db"] == True:
            ...
            rlog.info(f"{listx.coll} :: insert done, no doc posted to db.")
...
//250329
db.organization.aggregate([{ $match: {} }, { $out: "bk_organization_250331" }])
